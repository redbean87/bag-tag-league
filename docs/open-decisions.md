...
### 2. Club Member List Population

**Status:** Needs input

How should the club member list be initially populated?
...
5. **No player authentication.** Players identify themselves by selecting from the club member list.
...
10. **Tag redistribution is pool-based.** All players in the weekly sheet are included. Lowest score first, lower `in_tag` breaks ties, tags redistribute ascending from the pool. See ADR-003.
11. **`ClubMembers.current_tag` is a last-known calculated value.** It is not guaranteed to represent the player's physical tag. Players may trade tags between league days. The next check-in must require the player's actual `in_tag`.

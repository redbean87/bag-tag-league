...
- Google Sheets row numbers are ephemeral and must not be used as identifiers; use UUIDs instead (except `member_number`, which is a sequential integer, and `WeeklyPlayerRecords` which uses `member_number` within each weekly sheet)
...
- `in_tag` is the player's starting bag tag for the weekly league. It may be supplied by the player during check-in, or it may be blank if the player was added via UDisc import. Blank `in_tag` values are excluded from the tag pool but the player is still included in the ranking.
- `out_tag` is the player's ending bag tag, calculated by the app. The calculation includes every player in the weekly sheet regardless of `checked_in` status. The tag pool is built from valid `in_tag` values in the sheet, sorted ascending, and assigned to players ranked by lowest score (lower `in_tag` breaks ties). If the tag pool has fewer tags than players, the remaining players receive no `out_tag`.
